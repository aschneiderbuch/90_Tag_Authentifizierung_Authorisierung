...
export const login = async (req, res) => {
    ...
    try {
        const db = await getDb()
        const result = await db.collection(COL).findOne(user) // ! wir suchen nach dem User in der Datenbank/Dokument
        ...
        if (result === null) {      
            ...
            res.status(200).json({ token: mailtoken }) // wir geben den mailtoken ans FrontEnd zurück  
